...
ap.add_argument("-m", "--model", type=str, default = path+ 'NeuralNet/my_logs/SYNTHsimple/bestRunning_model.hdf5',
...
################################################
#%%
X, Y_label, Y_len,  N = loadImgsWithLabels(full=False, synth=False)
X2, Y2_label, Y2_len,  N2 = loadImgsWithLabels(full=False, synth="Synth")
...
Y_label_train = np.append(Y_label_train, Y2_label, axis=0)
Y_len_train = np.append(Y_len_train, Y2_len, axis=0)
print(X_train.shape)

order = np.random.permutation(np.array(range(trainID+N2)))
X_train = X_train[order]
Y_label_train = Y_label_train[order]
Y_len_train = Y_len_train[order]

X_dev = X[trainID:]
Y_label_dev = Y_label[trainID:]
Y_len_dev = Y_len[trainID:]
...
# model = keras.Model(inputs=[inputs,],outputs=inner)
# model.summary()
...
viz_cb = VizCallback(run_name, test_func, generatorVal.get_batch(training=True))
early_stopping= keras.callbacks.EarlyStopping(monitor='val_loss', patience=12, restore_best_weights=True, verbose=1)
checkpoint = keras.callbacks.ModelCheckpoint(os.path.join(LOGDIR, run_name) + "/bestRunning_model.hdf5", 
                                             monitor='val_loss', verbose=1,
                                             save_best_only=True, mode='auto')
rlrop = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.25, patience=5, verbose=1, mode='auto',
                                          min_delta=0.0001, cooldown=0, min_lr=0,)

#%%
history = model.fit(generatorTrain.get_batch(training=True), epochs=150, 
